package com.suqir.wasaischedule.ui.schedule_settings
...
class ScheduleSettingsActivity : BaseListActivity(), ColorPickerFragment.ColorPickerDialogListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        textWatcher = object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                if (showItems.size == 1) {
                    mRecyclerView.longSnack("没有找到结果，换个关键词试试看")
                }
            }
        }
    ...
    private fun onItemsCreated(items: MutableList<BaseSettingItem>) {
        ...
        items.add(HorizontalItem("上课时间表", "点击修改", listOf("时间表")))
    ...
    private fun onSeekBarItemClick(item: SeekBarItem, position: Int) {
        ...
        seekBar?.let {
            ...
            it.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                    dialog.setTitle("${item.title}：${seekBar?.progress?.plus(item.min)}")
                }
        ...
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
            ...
            val valueInt = process?.plus(item.min)!!
            when (item.title) {
